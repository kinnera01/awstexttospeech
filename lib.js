...
exports.generateSpeech_us = (strParts, opts) => {
  // Add in the default options.
  /* istanbul ignore next */
  opts = Object.assign({}, {
    'access-key': 'AKIAIBUPJLKDOKNWIKRA',
    ...
    'sample-rate': '22050',
    'secret-key': 'Uzquf0s1okfl5C3pMEAn+MOXbZC/K5viMZYRRZYu',
    type: opts.type || 'ssml',
    voice: opts.voice || 'Joanna'||'Raveena'||'Amy'
  },opts)
  //console.log(opts)
  /* istanbul ignore next */
  ...
  /* istanbul ignore next */
  debug('generateSpeech_us')(`Options: ${JSON.stringify(this.sanitizeOpts(opts))}`) // eslint-disable-line no-invalid-this
...
exports.generateSpeech_bri = (strParts, opts) => {
  // Add in the default options.
  /* istanbul ignore next */
  opts = Object.assign({}, {
    'access-key': 'AKIAIBUPJLKDOKNWIKRA',
  ...
  /* istanbul ignore next */
  debug('generateSpeech_bri')(`Options: ${JSON.stringify(this.sanitizeOpts(opts))}`) // eslint-disable-line no-invalid-this
...
//forindian accent
// console.log(generateSpeech)
exports.generateSpeech_ind = (strParts, opts) => {
  // Add in the default options.
  /* istanbul ignore next */
  opts = Object.assign({}, {
    'access-key': 'AKIAIBUPJLKDOKNWIKRA',
 ...
 // console.log(opts)
  ...
  /* istanbul ignore next */
  debug('generateSpeech_ind')(`Options: ${JSON.stringify(this.sanitizeOpts(opts))}`) // eslint-disable-line no-invalid-this
...
let chunkText = (text, maxCharacterCount) => {
  ...
  var arr = [];
  function addpause(val) {
    var breakword = [];
    var array1 = val.split(".");
    //console.log(array1.length)
    for (var i = 0; i < array1.length; i++) {
      var newword = array1[i] + "<break time='1ms'/>";
      // console.log("nw"+newword);
      breakword.push(newword)
    }
    //  console.log(breakword)
    return val = breakword.join(".");
  }
    for (var i = 0; i < parts.length; i++) {
   parts[i] = addpause(parts[i]);
      var newline = "<speak><prosody rate='slow'>"+parts[i] +"</prosody></speak>" ;
      arr.push(newline)
    }
 parts=arr;
 //console.log(parts)
...
exports.readText = (inputFilename, proc) => {
  ...
  return new Promise((resolve, reject) => {
  //  console.log("name : "+inputFilename)
    if (inputFilename) {
      // Read from a file.
...
exports.splitText = (text, maxCharacterCount, opts) => {
 ...
 // console.log(text)
  ...
  fs.writeFile("file-decode.xml", text, function (err) {
    if (err) {
